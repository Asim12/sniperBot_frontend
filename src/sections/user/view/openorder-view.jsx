...
export default function OpenOrders() {
  ...
  useEffect(() => {
    const getAllOpenOrders = async () => {
      try {
        const response = await callApi('/getBuyOrder', 'POST');
        console.log(
          '🚀 ~ file: user-view.jsx:117 ~ getAllOpenOrders ~ response:',
          response.data.buy_order
        );
        if (response.status === 200) {
          setLoading(false);
          dispatch({ type: 'SET_USERS', payload: response.data.buy_order });
        }
      } catch (error) {
        console.log(error);
      }
  ...
  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Buy Open Orders</Typography>
            ...
            <Table sx={{ minWidth: 800 }}>
              <UserTableHead
                ...
                headLabel={[
                  ...
                  { id: 'contractAddress', label: 'Address' },
